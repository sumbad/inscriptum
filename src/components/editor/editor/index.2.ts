...
export class InsSyntaxCodeBlock extends CodeBlock {
  ...
  highlight(highlight) {
    ...
    console.log('[InsSyntaxCodeBlock][highlight].this.domNode.this.cachedText !== text', this.cachedText !== text);
    console.log('[InsSyntaxCodeBlock][highlight].this.domNode.this.cachedText', this.cachedText);
    console.log('[InsSyntaxCodeBlock][highlight].this.domNode.text', text);
    ...
    if (this.cachedText !== text) {
      if (text.trim().length > 0 || this.cachedText == null) {
        ...
        let lines = '';
        const l = highlight(text).split('\n');

        for (let i = 0; i < l.length - 1; i++) {
          // const element = array[index];
          console.log(1111111111, l[i]);
          
          lines += `<span class="l">${l[i] || ' '}</span>\n`;
        }
        lines += `<span class="l">${l[l.length - 1]}</span>`;
        ...
        //     // el.innerHTML= elHTML;
        //     // const lineEnd: Node = Parchment.create('code-end') as any;
        //     // el.append(lineEnd);
        //     // this.domNode.append(el);

        //     lines += `<span class="l">${elHTML}</span>\n`;
        //   }
        // );

        this.domNode.innerHTML = `<code class="hljs">${lines}</code>`;
