...
(function () { var b = document.querySelector('time'); if (b && T.datetime) { var a = new Date(1E3 * T.datetime), d = 'January February March April May June July August September October November December'.split(' ')[a.getMonth()], c = a.getDate(); b.innerText = d + ' ' + (10 > c ? '0' : '') + c + ', ' + a.getFullYear() } })();
      ...
      keyboard: {
        bindings: {
          ...
          'code': {
            key: 192, // `
            handler: function (range, context) {
              console.log('keyboard.bindings.code.handler');
...
function getPageContent(for_draft) {
  ...
  $node.querySelectorAll('textarea,input').forEach(
    (el) => {
      el.nodeValue = el.getAttribute('data-value');
      el.removeAttribute('data-value');
    }
  );

  updateEditableText($node, false);
  // ($node.querySelector('[contenteditable]') as HTMLElement).removeAttribute('contenteditable');
  $node.removeAttribute('contenteditable');
  $node.removeAttribute('data-placeholder');
  $node.removeAttribute('data-label');
  $node.removeAttribute('data-title');

  const editable_text = $node.querySelector('.editable_text');
  editable_text && editable_text.classList.remove('editable_text');
  const focus = $node.querySelector('.focus');
  focus && focus.classList.remove('focus');
  const empty = $node.querySelector('.empty');
  empty && empty.classList.remove('empty');
  const emptyCssClass = $node.querySelector('[class=""]');
  emptyCssClass && emptyCssClass.removeAttribute('class');
  const file_progress = $node.querySelector('.file_progress');
  file_progress && file_progress.remove();
  const cursor_wrapper = $node.querySelector('.cursor_wrapper');
  cursor_wrapper && cursor_wrapper.remove();
  if (!for_draft) {
    ...
    context.querySelectorAll('.editable_text')
      .forEach(
        (el) => {
          ...
          if (el.classList.contains('.editable_input')) {
...
function updateEditable(is_editable) {
  ...
  if (!is_editable && $tl_content && $tl_header) {
    var title = ($tl_content.querySelector('h1') as HTMLElement).innerText;
    var author = ($tl_content.querySelector('address') as HTMLElement).innerText;
    var author_url = ($tl_content.querySelector('address a') as HTMLElement).getAttribute('href');
    ($tl_header.querySelector('h1') as HTMLElement).innerText = title;
    ($tl_header.querySelector('address a') as HTMLElement).innerText = author;
    if (author_url) {
